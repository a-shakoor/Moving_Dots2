...
pauseBeforeSingleDotMotion = dotInfo.pauseBeforeSingleDotMotion;
singleDotOutput = dotInfo.singleDotOutput;
...
while continue_show
   ...
   %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%   
    ...
    for df = 1:dotInfo.numDotField
        ...
        %black out around the fixation
        blackoutRadius = 3 * screenInfo.ppd;
        Screen('FillOval', curWindow, [0 0 0], ...
            [center(df,1)-blackoutRadius, center(df,2)-blackoutRadius, ...
            center(df,1)+ blackoutRadius, center(df,2)+blackoutRadius]);        
...
if isSingleDotTrial == 2 % saccade
    saccadeDot(screenInfo, 20, .6, 1); %20 degrees, .6 sec 
    KbQueueRelease();
    % Checks for code after singleDot ends
    initDecisionTimeFrames = round(decisionMaxTime * screenInfo.monRefresh);
    decisionTimeFrames = initDecisionTimeFrames;
...
if isSingleDotTrial == 1
    ...
    outputStruct.singleDotInitialY = -1;
    outputStruct.singleDotVelocity = -1;
    outputStruct.cohSingleDotCongruent = -1;
    outputStruct.timeSingleDotOn = -1;
    outputStruct.timeSingleDotBackToCenter = -1;
    outputStruct.timeSingleDotOff =  -1;
    outputStruct.timeResponse =  -1;
    outputStruct.startTimeSystem = -1;
    outputStruct.startTimePupil = -1;
end
