...
expName = 'testDots'  # from the Builder filename that created this script
...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath='C:\\Users\\Lab\\Documents\\GitHub\\Moving_Dots2\\Psychopy\\testDots_lastrun.py',
...
dots_stimulus = visual.DotStim(
    ...
    fieldPos=(0.0, 0.0), fieldSize=12,fieldShape='circle',
...
for thisTrial in trials:
    ...
    trialComponents = [break_text, postbreak_text, fixation1, dots_stimulus, blackout, red_dot, fixation2, pursuit_or_saccade_dot, response]
    ...
    while continueRoutine:
        ...
        if dots_stimulus.status == NOT_STARTED and fixation1.status==FINISHED:
            # keep track of start time/frame for later
        ...
        if dots_stimulus.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > dots_stimulus.tStartRefresh + 0.3-frameTolerance:
                # keep track of stop time/frame for later
                dots_stimulus.tStop = t  # not accounting for scr refresh
                dots_stimulus.frameNStop = frameN  # exact frame index
                win.timeOnFlip(dots_stimulus, 'tStopRefresh')  # time at next scr refresh
                dots_stimulus.setAutoDraw(False)
        
        # *blackout* updates
        if blackout.status == NOT_STARTED and dots_stimulus.status == STARTED:
            # keep track of start time/frame for later
        ...
        if blackout.status == STARTED:
            if bool(dots_stimulus.status == FINISHED):
                # keep track of stop time/frame for later
                blackout.tStop = t  # not accounting for scr refresh
                blackout.frameNStop = frameN  # exact frame index
                win.timeOnFlip(blackout, 'tStopRefresh')  # time at next scr refresh
        ...
        # *fixation2* updates
        if fixation2.status == NOT_STARTED and dots_stimulus.status == FINISHED:
            # keep track of start time/frame for later
        ...
        # *response* updates
        waitOnFlip = False
        if response.status == NOT_STARTED and (pursuit_trial == 1 or saccade_trial == 1) and (pursuit_or_saccade_dot.status==FINISHED and response_trial == 1):
            # keep track of start time/frame for later
        ...
        if response.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > response.tStartRefresh + 8-frameTolerance:
                # keep track of stop time/frame for later
    ...
    trials.addData('dots_stimulus.started', dots_stimulus.tStartRefresh)
    trials.addData('dots_stimulus.stopped', dots_stimulus.tStopRefresh)
    trials.addData('blackout.started', blackout.tStartRefresh)
    trials.addData('blackout.stopped', blackout.tStopRefresh)
